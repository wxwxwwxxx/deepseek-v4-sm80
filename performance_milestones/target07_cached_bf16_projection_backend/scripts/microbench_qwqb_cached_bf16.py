...
def _bench_owner_m(
    *,
    ...
    return {
        ...
        "cached_bf16_total_ms": cached_bf16_total_ms,
...
def _render_markdown(report: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# q_wqb Cached BF16 Projection Microbench")
    lines.append("")
    ...
    for row in report["results"]:
        ...
        lines.append(
            "| `{owner}` | {m} | {k} | {n} | `{current:.4f}` | `{intrinsic:.4f}` | `{fallback:.4f}` | `{cached_linear:.4f}` | `{cached_total:.4f}` | `{speedup:.2f}x` | `{max_abs:.6f}` | `{max_rel:.6f}` | `{ok}` |".format(
                ...
                cached_total=row["cached_bf16_total_ms"],
...
def main() -> None:
    ...
    parser.add_argument("--owners", default="attn.q_wqb.layer0")
    parser.add_argument("--m-values", default="1,4,8,16")
    parser.add_argument("--warmup", type=int, default=10)
    parser.add_argument("--iters", type=int, default=50)
    parser.add_argument("--seed", type=int, default=758)
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "performance_milestones/target07_cached_bf16_projection_backend/raw/qwqb_cached_bf16_microbench.json"
        ),
    )
    parser.add_argument(
        "--md-out",
    ...
    for owner_idx, owner in enumerate(owners):
        ...
        for m in m_values:
            ...
            print(
                ...
                f"cached_total={row['cached_bf16_total_ms']:.4f} ms, "
