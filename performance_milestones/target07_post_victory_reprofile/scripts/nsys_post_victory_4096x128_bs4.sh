...
MILESTONE_DIR="${MILESTONE_DIR:-${MINISGL_ROOT}/performance_milestones/target07_post_victory_reprofile}"
...
OUTPUT_DIR="${OUTPUT_DIR:-/tmp/dsv4_target0763_nsys_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}}"
NSYS_BASE="${NSYS_BASE:-/tmp/nsys_target0763_post_victory_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}}"
...
if [[ -f "${OUTPUT_DIR}/summary.json" ]]; then
  cp "${OUTPUT_DIR}/summary.json" \
    "${MILESTONE_DIR}/summaries/mini_post_victory_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}_nsys_summary.json"
fi
...
for sqlite in "${NSYS_BASE}"_rank*.sqlite; do
  ...
  python "${MILESTONE_DIR}/scripts/summarize_post_victory_nsys.py" "${sqlite}" \
    --json-out "${MILESTONE_DIR}/summaries/${rank_name}_classified.json" \
    --md-out "${MILESTONE_DIR}/summaries/${rank_name}_classified.md"
done
