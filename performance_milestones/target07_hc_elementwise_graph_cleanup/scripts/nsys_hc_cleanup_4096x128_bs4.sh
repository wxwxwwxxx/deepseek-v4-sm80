...
MILESTONE_DIR="${MILESTONE_DIR:-${MINISGL_ROOT}/performance_milestones/target07_hc_elementwise_graph_cleanup}"
...
VARIANT="${VARIANT:-dsv4_sm80_a100_victory_hccleanup}"
RUN_TAG="${RUN_TAG:-target0768_${VARIANT}_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}}"
...
RANK_WRAPPER="${RANK_WRAPPER:-${MINISGL_ROOT}/performance_milestones/target07_graph_layout_replay_deforestation/scripts/nsys_rank_wrapper.sh}"
BUCKET_SUMMARIZER="${BUCKET_SUMMARIZER:-${MINISGL_ROOT}/performance_milestones/target07_post_shared_expert_reprofile/scripts/summarize_post_shared_expert_nsys.py}"
DIRECT_COPY_CLASSIFIER="${DIRECT_COPY_CLASSIFIER:-${MINISGL_ROOT}/performance_milestones/target07_post_shared_expert_reprofile/scripts/classify_direct_copy_owners.py}"
...
for sqlite in "${NSYS_BASE}"_rank*.sqlite; do
  ...
  python "${BUCKET_SUMMARIZER}" \
    "${sqlite}" \
    --json-out "${MILESTONE_DIR}/summaries/${rank_name}_bucket_summary.json" \
    --md-out "${MILESTONE_DIR}/summaries/${rank_name}_bucket_summary.md" \
    --top 40
done
