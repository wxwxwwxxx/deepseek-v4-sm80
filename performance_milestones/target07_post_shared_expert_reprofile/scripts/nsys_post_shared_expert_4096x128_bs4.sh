...
MILESTONE_DIR="${MILESTONE_DIR:-${MINISGL_ROOT}/performance_milestones/target07_post_shared_expert_reprofile}"
...
RUN_TAG="${RUN_TAG:-target0767_${VARIANT}_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}}"
...
for sqlite in "${NSYS_BASE}"_rank*.sqlite; do
  ...
  python "${MILESTONE_DIR}/scripts/summarize_post_shared_expert_nsys.py" \
    "${sqlite}" \
