...
OUTPUT_DIR="${OUTPUT_DIR:-/tmp/dsv4_target0764_nsys_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}}"
NSYS_BASE="${NSYS_BASE:-/tmp/nsys_target0764_metadatadeforest_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}}"
...
if [[ -f "${OUTPUT_DIR}/summary.json" ]]; then
  cp "${OUTPUT_DIR}/summary.json" \
    "${MILESTONE_DIR}/summaries/mini_metadatadeforest_${PROMPT_LEN}x${DECODE_LEN}_bs${BATCH_SIZE}_np${NUM_PAGES}_nsys_summary.json"
fi

for sqlite in "${NSYS_BASE}"_rank*.sqlite; do
  [[ -f "${sqlite}" ]] || continue
  rank_name="$(basename "${sqlite%.sqlite}")"
  python "${MILESTONE_DIR}/scripts/summarize_decode_metadata_nsys.py" "${sqlite}" \
    --json-out "${MILESTONE_DIR}/summaries/${rank_name}_decode_metadata_subboundary.json" \
    --md-out "${MILESTONE_DIR}/summaries/${rank_name}_decode_metadata_subboundary.md"
done
