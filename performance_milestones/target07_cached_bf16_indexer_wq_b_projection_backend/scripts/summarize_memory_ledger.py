...
def _compute(
    qwqb_wob_idxwqb: dict[str, Any],
    qwqb_wob_baseline: dict[str, Any] | None,
    exact_baseline: dict[str, Any] | None,
) -> dict[str, Any]:
    config = qwqb_wob_idxwqb.get("config", {})
    num_pages = int(config.get("num_pages") or 0)
    page_size = int(config.get("page_size") or 0)
    metrics = (
        qwqb_wob_idxwqb.get("metrics")
        if isinstance(qwqb_wob_idxwqb.get("metrics"), dict)
        else qwqb_wob_idxwqb
    )
    ...
    q_prepare = _cache_prepare(qwqb_wob_idxwqb, "q_wqb_bf16_weight_cache")
    wo_prepare = _cache_prepare(qwqb_wob_idxwqb, "wo_b_bf16_weight_cache")
    idx_prepare = _cache_prepare(qwqb_wob_idxwqb, "indexer_wq_b_bf16_weight_cache")
    q_row = _owner_row(
        "attn.q_wqb",
        q_prepare,
    ...
    total_bytes = (
        q_row["extra_bytes_per_rank"]
        + wo_row["extra_bytes_per_rank"]
        + idx_row["extra_bytes_per_rank"]
    )
    ...
    return {
        "owners": [q_row, wo_row, idx_row],
        "total": {
            "owner": "total",
            "layers_cached": (
                q_row["layers_cached"] + wo_row["layers_cached"] + idx_row["layers_cached"]
            ),
        ...
        "memory_delta_vs_qwqb_wob_baseline": _memory_delta(qwqb_wob_idxwqb, qwqb_wob_baseline),
        "memory_delta_vs_exact_baseline": _memory_delta(qwqb_wob_idxwqb, exact_baseline),
        "qwqb_wob_idxwqb_metrics": _metrics(qwqb_wob_idxwqb),
        "qwqb_wob_baseline_metrics": _metrics(qwqb_wob_baseline),
        "exact_baseline_metrics": _metrics(exact_baseline),
        "graph_status": _graph_status(qwqb_wob_idxwqb),
        "prepare_report": _prepare(qwqb_wob_idxwqb),
    }
...
def _render_md(
    ...
    *,
    qwqb_wob_idxwqb_path: Path,
    qwqb_wob_baseline_path: Path | None,
    exact_baseline_path: Path | None,
) -> str:
    lines: list[str] = []
    lines.append("# q_wqb + wo_b + indexer.wq_b Cached BF16 Memory Ledger")
    lines.append("")
    lines.append(f"- q_wqb + wo_b + indexer.wq_b report: `{qwqb_wob_idxwqb_path}`")
    if qwqb_wob_baseline_path is not None:
        lines.append(f"- q_wqb + wo_b baseline report: `{qwqb_wob_baseline_path}`")
    ...
    q_delta = summary["memory_delta_vs_qwqb_wob_baseline"]
    e_delta = summary["memory_delta_vs_exact_baseline"]
    lines.append(f"| peak allocated delta vs q_wqb+wo_b baseline | `{_fmt_int(q_delta['peak_allocated_delta'])}` |")
    lines.append(f"| peak reserved delta vs q_wqb+wo_b baseline | `{_fmt_int(q_delta['peak_reserved_delta'])}` |")
...
def main() -> None:
    ...
    parser.add_argument("--qwqb-wob-idxwqb-report", type=Path, required=True)
    parser.add_argument("--qwqb-wob-baseline-report", type=Path)
    parser.add_argument("--exact-baseline-report", type=Path)
    parser.add_argument(
        "--json-out",
        type=Path,
        default=Path(
            "performance_milestones/target07_cached_bf16_indexer_wq_b_projection_backend/summaries/qwqb_wob_idxwqb_memory_ledger.json"
        ),
    )
    parser.add_argument(
        "--md-out",
    ...
    qwqb_wob_idxwqb = _load_json(args.qwqb_wob_idxwqb_report)
    assert qwqb_wob_idxwqb is not None
    qwqb_wob_baseline = _load_json(args.qwqb_wob_baseline_report)
    exact_baseline = _load_json(args.exact_baseline_report)
    summary = _compute(qwqb_wob_idxwqb, qwqb_wob_baseline, exact_baseline)
    ...
    args.md_out.write_text(
        _render_md(
            summary,
            qwqb_wob_idxwqb_path=args.qwqb_wob_idxwqb_report,
            qwqb_wob_baseline_path=args.qwqb_wob_baseline_report,
