...
def _indexer_wq_b_layers(weight_map: dict[str, str]) -> list[int]:
    prefix = "layers."
    suffix = ".attn.indexer.wq_b.weight"
    layers: list[int] = []
...
def _render_markdown(report: dict[str, Any]) -> str:
    lines: list[str] = []
    lines.append("# indexer.wq_b Cached BF16 Projection Microbench")
    lines.append("")
    ...
    lines.append(f"- Checkpoint indexer layers: `{report['indexer_wq_b_layers']}`")
    lines.append(f"- Checkpoint indexer layer count: `{report['indexer_wq_b_layer_count']}`")
    lines.append(f"- M values: `{report['m_values']}`")
    lines.append(f"- Warmup/iters: `{report['warmup']}` / `{report['iters']}`")
    lines.append("- Scope: replicated indexer query projection only; no all-reduce is involved.")
    lines.append("")
    lines.append(
...
def main() -> None:
    ...
    parser.add_argument("--owners", default="indexer.wq_b.layer2")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "performance_milestones/target07_cached_bf16_indexer_wq_b_projection_backend/raw/indexer_wqb_cached_bf16_microbench.json"
        ),
    )
    parser.add_argument(
        "--md-out",
        type=Path,
        default=Path(
            "performance_milestones/target07_cached_bf16_indexer_wq_b_projection_backend/summaries/indexer_wqb_cached_bf16_microbench.md"
        ),
    )
    ...
    indexer_layers = _indexer_wq_b_layers(weight_map)
    ...
    report = {
        ...
        "m_values": m_values,
        "owners": owners,
        "indexer_wq_b_layers": indexer_layers,
        "indexer_wq_b_layer_count": len(indexer_layers),
        "tp_size": args.tp_size,
        "tp_rank": args.tp_rank,
        "warmup": args.warmup,
        "iters": args.iters,
