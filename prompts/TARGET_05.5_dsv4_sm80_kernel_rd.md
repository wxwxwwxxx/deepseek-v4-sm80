...
```bash
python -u debug/dsv4/benchmark/offline/deepseek_v4_perf_matrix.py \
...
| `moe_gate/hash_topk/mega_moe_pre_dispatch` | implemented V1 default variant with fused-compute first path | exact bf16 grouped MoE: Triton route metadata + guarded route-grouped fused compute to per-route output + deterministic route sum; fallback pipeline is fused grouped FP4 w1w3 producer + fused activation + grouped w2 + route sum; rank-local routed/shared sum before one TP all-reduce | `MINISGL_DSV4_SM80_V1_MOE` enables v0 whitelist + `MINISGL_DSV4_SM80_MOE_ROUTE`; `MINISGL_DSV4_SM80_MOE_ROUTE` remains individually opt-in | passed: route metadata CPU parity, sm80 Triton route parity, direct fused-compute CUDA routed expert parity, V1 reduce-once equivalence, wrapper/model/perf smoke tests 39 passed | current fallback -> V1 fused/pipeline grouped: 19.65x `decode_tiny`, 85.96x `decode_grouped`, 58.40x `prefill_grouped`; route metadata ~0.14 ms; grouped vs bf16 oracle max_abs 0.0/0.0/0.25 | TP=8 TARGET 06 smoke passed: v0_bf16 -> v1_moe elapsed 12.49s -> 6.39s, TTFT 8.48s -> 3.00s, prefill tok/s 1.92 -> 5.33, decode tok/s 0.75 -> 0.89, fallback wrapper calls 184336 -> 44672, `dequant_fp4_weight` 59856 -> 0. Fused-guard V1-only smoke also passed: elapsed 5.94s, decode tok/s 2.80, `dequant_fp4_weight` 0, no grouped optional-none skips. | make grouped MoE the default path for the V1 variant and try fused compute first where shape guard avoids w1/w3 recompute explosion; do not globally promote yet because full decode_throughput_bs8/mixed_prefill/4096x1024 gates still need long-run reports | Adds `MINISGL_DSV4_SM80_V1_MOE`, perf/text/e2e `v1_moe` variant, local Triton route metadata kernels, guarded single-kernel fused compute to per-route output, deterministic route sum, fallback V1 pipeline, and V1 reduce boundary. Artifacts `/tmp/dsv4_moe_route_dispatch_bf16_grouped_v1_fused_compute_microbench.json`, `/tmp/dsv4_sm80_v1_moe_perf_smoke/summary.json`, `/tmp/dsv4_sm80_v1_moe_fused_compute_smoke/summary.json`. Current quant-act fallback differs from bf16-direct on larger hidden shapes by design; V2 INT8 remains separate opt-in only. |
