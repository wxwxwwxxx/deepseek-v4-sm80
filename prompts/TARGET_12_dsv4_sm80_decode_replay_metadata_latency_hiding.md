...
| TARGET 12.606 CUDA Graph Recipe Promotion And Cleanup | current | Wire the selected balanced, low-M/memory, and long-context recipes; publish a capacity-aware DGX A100 performance card across M=4/16/64/128/256 and 1K/4K/16K contexts; run bounded 512K/1M gates; clean stale diagnostics. Prompt: `prompts/TARGET_12.606_dsv4_sm80_cuda_graph_recipe_promotion_cleanup.md`. |
| TARGET 12.61 Workload And Large-M Backend Envelope Census | planned | Consume the performance card, profile exact/padded M and committed-context scaling, compare mini subgraphs and dispatch with SGLang/vLLM, and open long-context or large-M kernel rewrites only for measured material owners. Prompt: `prompts/TARGET_12.61_dsv4_sm80_workload_backend_envelope_census.md`. |
