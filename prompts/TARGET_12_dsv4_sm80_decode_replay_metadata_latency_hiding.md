...
| TARGET 12.2 Safe Timing And Kernel Census | active | Build graph-safe counters/microbench to split `prepare_for_replay` without CUDA events that crash replay; identify CPU/object assembly vs small Triton/D2D owners. Prompt: `prompts/TARGET_12.2_dsv4_sm80_decode_replay_safe_timing_kernel_census.md`. |
| TARGET 12.3 Low-Risk Replay Deforestation | todo | Apply low-risk fixes: cache env decisions, remove inactive branches, batch small copies, avoid per-step object churn, and skip unnecessary metadata creation. |
| TARGET 12.4 SGLang-Style In-Graph Metadata Prep PoC | todo | Prototype raw decode metadata plus in-graph full metadata materialization for decode, aligned with SGLang's `SGLANG_PREP_IN_CUDA_GRAPH` route. |
