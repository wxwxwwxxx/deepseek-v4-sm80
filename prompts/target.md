...
| TARGET 11 | `prompts/TARGET_11_dsv4_sm80_mtp_speculative_decoding.md` | active MTP route | MTP weights/oracle, V1 sidecar, and frozen-KV rollback correctness passed; TARGET 11.29 fixed target-verify metadata/bookkeeping but fail-closed on missing online C128 MTP lifecycle, so next is TARGET 11.295 before graph/perf. |
