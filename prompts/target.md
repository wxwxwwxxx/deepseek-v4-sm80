...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.67 promoted 复测 4096/1024/batch4 为 `131.6263 output tok/s`。07.71 已拒绝 HC/router exact-ish 精度路线，选择 vLLM-aligned FP8/custom projection-cache cluster；下一步运行 TARGET 07.72。 |
...
| TARGET 07.72 | `prompts/TARGET_07.72_dsv4_sm80_vllm_aligned_fp8_custom_projection_cache_boundary.md` | next todo | vLLM-Aligned FP8 / Custom Projection-Cache Boundary：围绕 WQA/WKV、shared experts、`wo_a`、`q_wqb`、`wo_b`、indexer projection pieces 等 coherent cluster 做 opt-in FP8/custom boundary 实验；不做完整 `fp8_ds_mla` KV-cache E2E，不改 HC/router 精度，profile gate 为 projection/GEMM 至少降 `0.10s`，macro gate 为 4096/1024 `>=3%`。 |
