...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 best exact 为 Marlin WNA16 + global topk/lens + bf16 split-K sparse decode，4096/1024/batch4 达到 `68.81 output tok/s`，仍低于旧 serving 胜利线 `114.07 output tok/s`。下一步运行 TARGET 07.43。 |
...
| TARGET 07.43 | `prompts/TARGET_07.43_dsv4_sm80_vllm_ablation_before_precision.md` | next todo | 在进入 07.50 前，对 vLLM 做 aux stream、persistent topk/indexer、CUDA graph 的 env-gated 破坏性实验，用 vLLM 自身掉速来判断是否应先 port/adapt 核心机制。 |
