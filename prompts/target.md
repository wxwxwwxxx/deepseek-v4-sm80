...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.67 promoted 复测 4096/1024/batch4 为 `131.6263 output tok/s`。07.70 证明 narrow exact BF16 small-GEMM layout/backend 路线 microbench 有信号但 profile/macro 基本不动；下一步运行 TARGET 07.71 做 precision/boundary pivot。 |
...
| TARGET 07.71 | `prompts/TARGET_07.71_dsv4_sm80_precision_boundary_pivot.md` | next todo | Precision / Boundary Pivot：基于 07.70 负结果，短目标评估两条下一阶段路线：exact-ish HC/router FP32/TF32/BF16 ownership，或 vLLM-aligned FP8 projection/cache/fused indexer/rope/quant/custom boundary；只做 probe 和决策，不在同一线程实现完整大路线。 |
