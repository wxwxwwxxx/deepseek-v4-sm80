...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 best exact 为 Marlin WNA16 + global topk/lens + bf16 split-K sparse decode，4096/1024/batch4 达到 `68.81 output tok/s`；当前 opt-in cached BF16 `q_wqb + wo_b + indexer.wq_b` 路径达到 `105.7645 output tok/s`。07.61 选出下一刀为 `attn.wo_a` attention boundary；下一步运行 TARGET 07.62，对齐 vLLM SM80 per-group BF16 BMM/cache 边界。 |
...
| TARGET 07.62 | `prompts/TARGET_07.62_dsv4_sm80_wo_a_attention_boundary_parity.md` | next todo | 对齐 vLLM SM80 `wo_a` per-group BF16 BMM/cache 设计：实现 opt-in `wo_a` BF16 grouped BMM cache，避免 decode-time dequant/layout/einsum，记录显存账、text smoke、macro、owner profile，并在收益不足时停止。 |
