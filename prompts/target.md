...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 best exact 为 Marlin WNA16 + global topk/lens + bf16 split-K sparse decode，4096/1024/batch4 达到 `68.81 output tok/s`；当前 opt-in cached BF16 `q_wqb + wo_b` 路径达到 `98.6953 output tok/s`。下一步运行 TARGET 07.60，把 cached BF16 dequantized weight backend 扩展到 `indexer.wq_b`，随后重新 profile 并选择新瓶颈。 |
...
| TARGET 07.60 | `prompts/TARGET_07.60_dsv4_sm80_cached_bf16_indexer_wq_b_projection_backend.md` | next todo | 在 q_wqb+wo_b cached BF16 基础上扩展到 `indexer.wq_b`。预计只缓存 21 个 C4/indexer 层，增量约 `0.3281 GiB/rank`；完成后三大 `_quantized_linear_fp8_kernel` owner 都应被处理，必须 fresh profile 后再决定是 vLLM parity、通信/overlap、activation fake-quant 还是 graph/layout。 |
