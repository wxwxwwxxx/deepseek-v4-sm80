...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.63 复测 4096/1024/batch4 达到 `119.4153 output tok/s`。07.64 metadata deforest helper 正确但未达 gate，保留 opt-in 不进 bundle；下一步运行 TARGET 07.65 做 direct-copy owner attribution。 |
...
| TARGET 07.65 | `prompts/TARGET_07.65_dsv4_sm80_direct_copy_owner_attribution.md` | next todo | Measurement-only direct-copy owner attribution：围绕 07.64 后仍占 `0.731834s` 的 `direct_copy`，在 `batch_forward` / `batch_forward_enqueue` 下添加 profiling-only NVTX 和 classifier，归因 graph input staging、replay metadata copy、layout materialization、attention/MoE/sampler staging 等具体 owner，再选择一个下一实现 target。 |
