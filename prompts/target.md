...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 best exact 为 Marlin WNA16 + global topk/lens + bf16 split-K sparse decode，4096/1024/batch4 达到 `68.81 output tok/s`，仍低于旧 serving 胜利线 `114.07 output tok/s`。07.51 证明 vLLM 的 FP8 indexer backend 值得 port/adapt；下一步运行 TARGET 07.52。 |
...
| TARGET 07.52 | `prompts/TARGET_07.52_dsv4_sm80_vllm_fp8_indexer_backend_port.md` | next todo | 将 vLLM 的 FP8 paged indexer backend 作为 opt-in 路径 port/adapt 到 mini：对齐 Q scale folding、FP8 indexer cache layout、`fp8_paged_mqa_logits_triton` decode 语义；先 microbench 达标，再跑 4096/128 和必要的 4096/1024 macro。 |
