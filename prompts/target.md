...
| TARGET 11 | `prompts/TARGET_11_dsv4_sm80_mtp_speculative_decoding.md` | active MTP route | MTP weights/oracle, V1 sidecar, and frozen-KV rollback correctness passed; TARGET 11.297 localized the remaining row0 drift to layer0 `wo_a` projection, so next is TARGET 11.298 before graph/perf. |
