...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 总路线图。主胜利线是 TP8/page size 256/4096 input/1024 output/batch4 超过旧 vLLM-based serving 基线 `114.07 output tok/s`，默认路径保持 exact。07.1/07.2/07.25 已形成 baseline、comm/graph 和 subgraph parity 证据；07.3/07.35/07.36 证明外围 MoE route/workspace/runner cut 没有解决 W13/W2 decode gap；下一步拆成 07.37 先判明 vLLM sm80 MoE backend，再根据结论选择 07.38 exact backend adaptation 或 07.4 precision lane。 |
...
| TARGET 07.37 | `prompts/TARGET_07.37_dsv4_sm80_moe_backend_identification.md` | planned | MoE backend identification：只做 vLLM sm80 MoE expert backend 判明、支持条件/weight layout/activation precision 分类、轻量 probe/microbench 和下一步决策；不做大规模 backend port。 |
| TARGET 07.38 | `prompts/TARGET_07.38_dsv4_sm80_moe_exact_backend_adapt.md` | planned | MoE exact expert backend adaptation：仅当 07.37 证明存在可移植 W4A16/bf16 activation backend 且预期 routed-MoE 至少 `1.5x` 时启动；复用 07.36 runner，只替换专家 backend，不改变默认精度。 |
