...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.67 promoted 复测 4096/1024/batch4 为 `131.6263 output tok/s`。07.68 HC cleanup 保留 opt-in、不推广；下一步运行 TARGET 07.69 重新归因当前 projection/GEMM backend/owner，避免沿用过期 07.57 结论。 |
...
| TARGET 07.69 | `prompts/TARGET_07.69_dsv4_sm80_projection_gemm_backend_owner_reattribution.md` | next todo | Projection/GEMM backend and owner re-attribution：在 07.66/07.67/07.68 后重新解释当前 promoted path 的 `0.778887s` projection/GEMM bucket，按 owner/backend 与 vLLM source boundary 对齐，并只选择一个证据充分的后续实现 target。 |
