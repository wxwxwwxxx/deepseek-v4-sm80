...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.67 promoted 复测 4096/1024/batch4 为 `131.6263 output tok/s`。07.75 已打通 mini-owned dense FP8 Marlin extension bridge，默认 mini torch ABI 下 focused owner gate PASS；下一步运行 TARGET 07.76，将该 bridge 接入 TP8 runtime opt-in 并跑 smoke/profile/macro。 |
...
| TARGET 07.76 | `prompts/TARGET_07.76_dsv4_sm80_mini_owned_fp8_marlin_projection_runtime.md` | next todo | Mini-Owned FP8 Marlin Projection Runtime：将 07.75 mini-owned bridge 替换 07.74 的 vLLM-helper runtime path，作为默认关闭 opt-in 接入 `q_wqb`、`wo_b local`、shared experts down；要求 TP8 text smoke、graph replay、4096/128 profile、4096/1024 macro 和显存生命周期 gate 后再决定是否 promote。 |
