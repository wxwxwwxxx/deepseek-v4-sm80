...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 best exact 为 Marlin WNA16 + global topk/lens + bf16 split-K sparse decode，4096/1024/batch4 达到 `68.81 output tok/s`；当前 opt-in FP8 indexer + FP8 activation quant Triton 路径达到 `87.08 output tok/s`。07.54 已证明 graph/layout custom-boundary 优化有效，但剩余 graph/layout 与 projection/GEMM 并列；下一步运行 TARGET 07.55 做最后一次 graph/layout 证据裁决，否则 pivot 到 projection/GEMM backend parity。 |
...
| TARGET 07.55 | `prompts/TARGET_07.55_dsv4_sm80_remaining_graph_layout_or_projection_pivot.md` | next todo | 对 07.54 后剩余 direct-copy、bf16/float8 copy、CatArray/index/gather、pow/mean/mul 节点做 vLLM 边界对照；只允许一个 focused graph/layout PoC，若不达标则输出 projection/GEMM backend parity 计划。 |
