...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 milestone opt-in 为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，包含 Marlin WNA16 MoE、CUDA graph、FP8 indexer cache、split-K sparse decode 和四个 BF16 projection cache；4096/1024/batch4 达到 `116.2553 output tok/s`，已超过旧 serving victory line `114.07`。下一步运行 TARGET 07.63 做 post-victory 确认、fresh profile 和下一瓶颈重排。 |
...
| TARGET 07.63 | `prompts/TARGET_07.63_dsv4_sm80_post_victory_reprofile_and_next_bottleneck.md` | next todo | post-victory 确认与 fresh bottleneck reset：统一使用 `dsv4_sm80_a100_victory`，复测 text smoke、4096/128、4096/1024、graph replay/eager decode、显存账和 nsys/owner profile；输出新的 top-bucket/vLLM parity 表，并只选择一个下一实现 target。 |
