...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 best exact 为 Marlin WNA16 + global topk/lens + bf16 split-K sparse decode，4096/1024/batch4 达到 `68.81 output tok/s`；当前 opt-in FP8 indexer + FP8 activation quant Triton 路径达到 `87.08 output tok/s`。07.57 已证明 projection/GEMM 的主合同是 `_quantized_linear_fp8_kernel`，下一步运行 TARGET 07.58，先做 `attn.q_wqb` cached BF16 dequantized weight path，并记录显存/KV token 成本。 |
...
| TARGET 07.58 | `prompts/TARGET_07.58_dsv4_sm80_cached_bf16_projection_backend.md` | next todo | opt-in cached BF16 dequantized weight backend。先只处理 `attn.q_wqb`，要求 cache 在 CUDA graph capture 前预分配/准备，decode replay 中只读不分配；同时记录 extra weight cache bytes/rank、peak memory delta、KV bytes/token，以及等价减少的 KV tokens/pages。 |
