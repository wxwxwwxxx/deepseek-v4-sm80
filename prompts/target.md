...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 总路线图。主胜利线是 TP8/page size 256/4096 input/1024 output/batch4 超过旧 vLLM-based serving 基线 `114.07 output tok/s`，默认路径保持 exact。精度路线是 bf16-direct 先行、不默认做 activation quantization、不静默降低原有 fp32，之后再评估 fp8/fp4 act quant 和 INT8 Tensor Core opt-in。该目标拆分为 07.1 公平复测与 vLLM execution diff、07.2 通信与 CUDA graph、07.3 MoE exact V2、07.4 precision lanes。 |
| TARGET 07.1 | `prompts/TARGET_07.1_dsv4_sm80_fair_rebench_vllm_diff.md` | planned | 先做公平 mini/vLLM 复测和 nsys/sqlite 对标，统一 warmup、TP8、page/block size 256、4096/1024 主胜利线和 4096/128 短 profile 线，并输出 `performance_milestones/target07_vllm_gap/`。 |
| TARGET 07.2 | `prompts/TARGET_07.2_dsv4_sm80_comm_cuda_graph.md` | planned | 在 07.1 基础上处理通信与 CUDA graph：标注 DSV4 all-reduce/all-gather 来源，修复/评估 PyNCCL，解除 DSV4 decode graph 禁用门禁，并对照 vLLM custom all-reduce/CUDA graph dispatcher。 |
| TARGET 07.3 | `prompts/TARGET_07.3_dsv4_sm80_moe_v2_exact.md` | planned | 在 07.1/07.2 证据确认 MoE 仍为主要瓶颈后，推进 exact MoE V2：引入 MoE execution plan、优化 grouped FP4 W13/W2、减少 MoE 小 kernel，并保持 INT8 仅为后续 opt-in 研究线。 |
| TARGET 07.4 | `prompts/TARGET_07.4_dsv4_sm80_precision_lanes.md` | planned | 精度路线实验：在 bf16-direct exact 路径足够强后，分别评估 fp8/fp4 activation quantization、TF32 对 fp32 matmul-like 工作的影响，以及 INT8 Tensor Core MoE opt-in；vLLM 的量化实现可作为优先参考，但不强制 mini 第一阶段对齐 vLLM 精度策略。 |
