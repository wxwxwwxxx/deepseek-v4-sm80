...
| TARGET misc release cleanup | `prompts/TARGET_misc_dsv4_release_cleanup.md` | active final release cleanup | Reduce the active repository to DeepSeek V4 Flash support, one default optimized path, one opt-in fallback/oracle path, a clean DSV4 benchmark/README surface, and a final package/correctness/performance soak.  Misc 01 proved both modes but its first manifest needs TARGET misc 01.5 hardening before deletion: kernel retention must be authorized by actual release/oracle runtime coverage, including private Triton/JIT/Marlin ownership. |
