...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 best exact 为 Marlin WNA16 + global topk/lens + bf16 split-K sparse decode，4096/1024/batch4 达到 `68.81 output tok/s`；当前 opt-in cached BF16 `q_wqb + wo_b + indexer.wq_b` 路径达到 `105.7645 output tok/s`。下一步运行 TARGET 07.61，重新对齐 mini 与 vLLM 的 macro/bucket/owner/source-boundary 证据，并据此选择下一实现目标。 |
...
| TARGET 07.61 | `prompts/TARGET_07.61_dsv4_sm80_post_cached_bf16_vllm_parity_reprofile.md` | next todo | 冻结三 owner cached BF16 mini baseline，和 vLLM 进行 macro/bucket/owner/source-boundary 对比，明确下一步是 `wo_a`/attention boundary、shared-expert layout、communication/overlap、precision/cache 还是重新补 profile。 |
