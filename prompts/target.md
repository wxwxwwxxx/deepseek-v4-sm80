...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 总路线图。主胜利线是 TP8/page size 256/4096 input/1024 output/batch4 超过旧 vLLM-based serving 基线 `114.07 output tok/s`，默认路径保持 exact。07.1/07.2/07.25/07.3/07.35/07.36/07.37/07.38/07.39/07.391 已形成证据链：mini 已拥有 exact Marlin WNA16 MoE backend，4096/1024/batch4 达到 `54.47 output tok/s`，但仍低于胜利线；短 Nsight 显示下一步应转 sparse attention/indexer/cache 或 metadata/runtime overhead，而不是 07.4 precision lane。 |
...
| TARGET 07.3 | `prompts/TARGET_07.3_dsv4_sm80_moe_v2_exact.md` | completed | exact MoE V2：引入 MoE execution plan、per-layer workspace 和 bf16-output SwiGLU cut；正确性通过，但 4096/1024/batch4 decode 几乎无收益，artifact 位于 `performance_milestones/target07_moe_v2/`。按 stop condition 转入 07.35。 |
| TARGET 07.35 | `prompts/TARGET_07.35_dsv4_sm80_post_moe_reparity.md` | completed | MoE V2 后重新做 mini/vLLM parity、宏观复测和短 profile-equivalent review，artifact 位于 `performance_milestones/target07_post_moe_reparity/`。结论：不要继续外围 MoE 小优化；下一步是 07.36 vLLM FusedMoE runner adaptation。 |
| TARGET 07.36 | `prompts/TARGET_07.36_dsv4_sm80_vllm_fused_moe_runner_adapt.md` | completed | vLLM FusedMoE runner adaptation：已实现 mini-owned runner boundary 并通过正确性/TP8 text smoke，但 4096/1024/batch4 仅约 `+0.16%`，fresh Nsight 仍显示 grouped FP4 W13/W2 dominant。Artifact 位于 `performance_milestones/target07_vllm_fused_moe_runner/`；结论是不再继续 MoE wrapper/runner polish。 |
| TARGET 07.37 | `prompts/TARGET_07.37_dsv4_sm80_moe_backend_identification.md` | completed | vLLM DeepSeek V4 sm80 MoE backend 已判明为 Marlin MXFP4 W4A16，bf16/fp16 activations，非 precision lane。Artifact 位于 `performance_milestones/target07_moe_backend_identification/`；结论是启动 07.38 exact backend adaptation。 |
| TARGET 07.38 | `prompts/TARGET_07.38_dsv4_sm80_moe_exact_backend_adapt.md` | completed | direct Marlin adaptation 停在 precise blocker：mini 没有 `gptq_marlin_repack` 和 `_moe_C::moe_wna16_marlin_gemm` custom-op surface。已添加 explicit Marlin opt-in guard，默认 grouped FP4 不变，不 silent fallback。Artifact 位于 `performance_milestones/target07_moe_exact_backend_adapt/`。 |
| TARGET 07.39 | `prompts/TARGET_07.39_dsv4_sm80_marlin_custom_op_bridge.md` | completed | Marlin custom-op bridge feasibility 完成：`/workspace/venvs/vllm-dsv4` 已编译 ops 可 import/call，T=4/T=4096 synthetic DSV4-like MoE 跑通，fused Marlin 相比 current grouped FP4 约 `4.55x`/`20.05x`。`vllm_marlin_bridge` 仅作为 explicit probe marker，mini runtime 仍报 unsupported，默认 grouped FP4 不变且无 silent fallback。Artifact 位于 `performance_milestones/target07_marlin_custom_op_bridge/`；下一步开 narrow mini-owned Marlin WNA16 csrc port target。 |
| TARGET 07.391 | `prompts/TARGET_07.391_dsv4_sm80_marlin_wna16_csrc_port.md` | completed | mini-owned Marlin WNA16 csrc port 完成：vendor narrow vLLM Marlin source surface，新增 `marlin_wna16` opt-in backend、weight transform/cache、TP8 text smoke、4096/128 与 4096/1024 macro、rank0 Nsight。4096/1024/batch4 达到 `54.47 output tok/s`，无 vLLM runtime dependency、无 silent fallback，但仍未达到旧 vLLM serving 基线 `114.07 output tok/s`。Artifact 位于 `performance_milestones/target07_marlin_wna16_csrc_port/`；下一步转 attention/indexer/cache 或 metadata/runtime overhead。 |
