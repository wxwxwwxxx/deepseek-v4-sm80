...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.66 已把 shared expert BF16 weight cache 纳入 victory bundle，4096/1024/batch4 达到 `131.7707 output tok/s`。下一步运行 TARGET 07.67 做 post-shared-expert reprofile，重新排序瓶颈后再选择实现目标。 |
...
| TARGET 07.67 | `prompts/TARGET_07.67_dsv4_sm80_post_shared_expert_reprofile.md` | next todo | 07.66 后的 promoted path 重新 profiling 和瓶颈重排：确认 `dsv4_sm80_a100_victory` 已包含 shared cache，抓 4096/128 rank0 profile，重排 communication、HC/elementwise、remaining direct-copy、sparse attention、MoE runner finalization 等候选项，再选择一个下一实现 target。 |
