...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.67 promoted 复测 4096/1024/batch4 为 `131.6263 output tok/s`。下一步运行 TARGET 07.68 做 HC / elementwise graph boundary cleanup，对齐 vLLM `mhc_pre` / `mhc_post` 边界。 |
...
| TARGET 07.68 | `prompts/TARGET_07.68_dsv4_sm80_hc_elementwise_graph_cleanup.md` | next todo | HC / elementwise graph boundary cleanup：围绕 mini `hc_pre_fallback` / `hc_post_fallback` 和 vLLM `torch.ops.vllm.mhc_pre` / `mhc_post` 做 source parity、microbench、一个 opt-in PoC；目标是降低 4096/128 rank0 `HC/elementwise + adjacent layout` 至少 `0.15s`，并争取 4096/1024 macro `>=3%`。 |
