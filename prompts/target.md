...
| TARGET 07 | `prompts/TARGET_07_dsv4_sm80_vllm_gap_closure.md` | planned | DSV4 sm80 vLLM gap closure 新主路线图。当前 promoted milestone 仍为 `dsv4_sm80_a100_victory` / `MINISGL_DSV4_SM80_A100_VICTORY_BUNDLE=1`，07.67 promoted 复测 4096/1024/batch4 为 `131.6263 output tok/s`。07.69 已证明旧 07.57 FP8 quantized-linear 瓶颈过期，当前 projection/GEMM 主线是 BF16 small-GEMM cluster；下一步运行 TARGET 07.70。 |
...
| TARGET 07.70 | `prompts/TARGET_07.70_dsv4_sm80_bf16_small_gemm_backend_cluster.md` | next todo | BF16 Small-GEMM Backend Cluster：围绕 07.69 识别出的 `0.521619s` BF16 small-GEMM cluster，尝试 cuBLASLt algo/splitK policy、prepacked layout、CUTLASS/Triton small-M kernel、owner-local grouped/fused GEMM 等 exact-route 方案；profile gate 为 projection/GEMM 至少降 `0.10s` 或 BF16 cluster 至少降 `20%`，macro gate 为 4096/1024 `>=3%`。 |
