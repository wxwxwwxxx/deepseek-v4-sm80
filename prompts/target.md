...
| TARGET 11 | `prompts/TARGET_11_dsv4_sm80_mtp_speculative_decoding.md` | active MTP route | MTP weights/oracle, V1 sidecar, and frozen-KV rollback correctness passed; TARGET 11.298 closed `wo_a` row0 parity, but bs=2/4 multi-request verify contract still blocks graph/perf, so next is TARGET 11.299. |
